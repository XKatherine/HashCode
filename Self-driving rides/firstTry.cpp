...
int isAvailable(Car c, Ride r){
	//from the current position to the start point
	int starttime = c.endTime + abs(c.location.first-r.start.first) + abs(c.location.second-r.start.second);
	if(starttime <= r.et)
		return r.et + r.path;
	if(starttime <= r.lt - r.path)
		return starttime+r.path;
	else
		return -1;
}
...
int algo(priority_queue<Ride>& rides, vector<Car>& cars, int R, int C, int F, int N, int B, int T){ 
	...
	while(!rides.empty()){
		...
		for(int i = 0; i < cars.size(); i++){
			int newEnd = isAvailable(cars[i],r);
			if(newEnd >=0){
			   cars[i].endTime = newEnd;
			   cars[i].location = r.end;
			   cars[i].ride.push_back(r.no);
			   if(newEnd - r.path == r.et)
				   score += B;
			   score += r.path;
			   break;
			}
